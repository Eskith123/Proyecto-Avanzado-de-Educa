...
const Carta: React.FC<CartaDisplayProps> = (props) => {
    ...
    return (
        <div
            onClick={() => !ocultarAcciones && props.onCardClick(props)}
            className={`max-w-72 w-full rounded-xl overflow-hidden shadow-2xl m-3 bg-gray-900 border-2 border-white-700 relative p-1 transition-all ${!ocultarAcciones ? 'hover:scale-105' : ''} ${props.estaSeleccionada ? 'ring-4 ring-red-600' : ''}`}
        >
